...
sub firewallrules_inventory_handler {
    ...
    foreach my $line (_getFirewallRules()) {
        ...
        if ($current_chain eq 'INPUT' || $current_chain =~ /ufw.*input/i) {
            $direction = "INPUT";
        } elsif ($current_chain eq 'OUTPUT' || $current_chain =~ /ufw.*output/i) {
            $direction = "OUTPUT";
        } elsif ($current_chain eq 'FORWARD' || $current_chain =~ /ufw.*forward/i) {
            $direction = "FORWARD";
        }
...
sub _getFirewallRules {
    ...
    for my $cmd (['iptables', 'IPv4'], ['ip6tables', 'IPv6']) {
        ...
        # Ajoute les sous-chaînes UFW si elles existent
        open(my $fh_chains, '-|', "$bin -L -n 2>/dev/null") or next;
        while (my $line = <$fh_chains>) {
            chomp $line;
            if ($line =~ /^Chain (ufw\S+)/) {
                push @chains, $1;
            }
        ...
        for my $chain (@chains) {
            ...
            # Utilise open() au lieu des backticks pour éviter la troncature des longues lignes
            open(my $fh, '-|', "$bin -L $chain -n -v 2>/dev/null") or next;
            while (my $line = <$fh>) {
                push @all_rules, $line;
            }
            close($fh);
